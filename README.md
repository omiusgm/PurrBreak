...
В настройках PurrBreak есть блок `Браузеры`: он показывает, какие браузеры найдены на этом Mac, где уже получилось прочитать активную вкладку, а где нужен доступ Automation.
...
- В настройках показывается статус браузеров: подключен, нужен доступ Automation, не найден или пока не поддерживается.

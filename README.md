...
При первом запуске PurrBreak покажет окно настройки браузеров. Открой YouTube в браузере, которым пользуешься, и вернись в это окно. macOS может попросить доступ Automation для браузера - его нужно разрешить.
...
Окно `Настройка браузеров` показывает, какие браузеры найдены на этом Mac, где уже получилось прочитать активную вкладку, а где нужен доступ Automation. Его можно открыть из главных настроек или из меню-бара.
...
- В отдельном окне показывается статус браузеров: подключен, нужен доступ Automation, не найден или пока не поддерживается.
